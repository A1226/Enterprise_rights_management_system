...
//        processBuilder.command("java","-jar","f:/xc-service-manage-course.jar");
    ...
    @Test
    public void testProcessBuilderOne() throws IOException {
        //创建ProcessBuilder对象
        ProcessBuilder processBuilder = new ProcessBuilder();
        //设置第三方应用程序的命令
        processBuilder.command("ping","127.0.0.1");
        ...
        char[] chars = new char[1024];
        int let = -1;
        while ((let = reader.read(chars)) != -1){
            String string = new String(chars,0,let);
            System.out.println(string);
        }
        inputStream.close();
        reader.close();
    ...
    public void testffmpeg() throws IOException {
        ...
        command.add("-i");
        command.add("D:\\upload\\lucene.avi");
        ...
        command.add("-r");
        command.add("18");
        command.add("D:\\upload\\lucene.mp4");
